...
PATCH_DEPLOY="{\"spec\":{\"template\":{\"spec\":{\"containers\": [{\"name\":\"api\", \"image\":\"${IMAGE}\", \"imagePullPolicy\":\"Always\"}]}}}}"
kubectl --kubeconfig ./napptive/default/napptive-kubeconfig patch deployments.apps nodejs-api -p "${PATCH_DEPLOY}"
